...
if (isServer) then {
    ...
    if (count _cargo > 0) then {
     ...
     _wait = [_obj,_carrier,[0,(15 - _yoffset),(-1.35 + _zoffset)],[0,-3 - (boundingBoxReal _obj select 1 select 1),(-1.35 + _zoffset)],0.1,0.01,180] spawn Lala_C17_fnc_attachToSmooth;
     waitUntil {scriptDone _wait};
     _wait2 = [_obj,_carrier,[0,-3 - (boundingBoxReal _obj select 1 select 1),(-1.35 + _zoffset)],[0,-19,(-3.45 + _zoffset)],0.1,0.01,180] spawn Lala_C17_fnc_attachToSmooth;
