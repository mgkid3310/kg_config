...
_height = (_bound select 1 select 2) + (boundingCenter _obj select 2);
...
if (count _near > 0) then {
    ...
    if ((_width <= 10) && (_length <= 22) && (_height <= 9) && (_weight <= 137250)) then {
...
/* private _obj = vehicle ACE_player;
if (_obj isEqualTo ACE_player) exitWith {hint "You're not in a Vehicle";};
[[_carrier, _obj], "Lala_C17_fnc_loadCargo"] spawn BIS_fnc_MP; */
