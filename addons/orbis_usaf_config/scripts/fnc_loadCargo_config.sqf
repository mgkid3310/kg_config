...
if ((_carrier modelToWorldVisual [0,-20,-3.45]) distance _obj > 15) then {
	...
	_carrier setVariable ["c17_cargo",((_carrier getVariable ["c17_cargo",[]]) + [_obj]),true];
	_zoffset = boundingCenter _obj select 2;
	...
	if (isServer) then {
		_wait = [_obj,_carrier,[0,-19,(-3.45 + _zoffset)],[0,-3 - (boundingBoxReal _obj select 1 select 1),(-1.35 + _zoffset)],0.1,0.01,	180] spawn Lala_C17_fnc_attachToSmooth;
		waitUntil {scriptDone _wait};
		_wait2 = [_obj,_carrier,[0,-3 - (boundingBoxReal _obj select 1 select 1),(-1.35 + _zoffset)],[0,(15 - _yoffset),(-1.35 + _zoffset)],0.1,0.01,	180] spawn Lala_C17_fnc_attachToSmooth;
...
// _obj attachTo [_carrier, (getPos _obj) vectorDiff (getPos _carrier)];
