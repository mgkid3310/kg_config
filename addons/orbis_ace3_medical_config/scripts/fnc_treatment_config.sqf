...
switch (_treatmentType) do {
    ...
    case ("Injection"): {
        _action = "Injecting";
        _treatment = getText (configProperties [configFile >> "ACE_Medical_Actions" >> _medicalLevel >> _className, "configName _x == 'treatmentDisplay'"] select 0);
    };
    case ("Others"): {
        _action = getText (configProperties [configFile >> "ACE_Medical_Actions" >> _medicalLevel >> _className, "configName _x == 'treatmentDisplay'"] select 0);
        _treatment = "";
    };
    default {
        _treatmentType = "none";
