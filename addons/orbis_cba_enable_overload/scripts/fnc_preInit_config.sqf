...
SLX_XEH_COMPILE = compile "compile preprocessFileLineNumbers _this"; //backwards comps
...
GVAR(EventsLowercase) = [];
{
    ...
    if (_x isEqualTo "Init") then {
        FUNC(Init) = compile "(_this select 0) call CBA_fnc_initEvents; (_this select 0) call CBA_fnc_init";
    } else {
        if (_x isEqualTo "HitPart") then {
            FUNC(HitPart) = compile format ['{call _x; nil} count ((_this select 0 select 0) getVariable QGVAR(%1))', _x];
        } else {
            missionNamespace setVariable [
                format [QFUNC(%1), _x],
                compile format ['{call _x; nil} count ((_this select 0) getVariable QGVAR(%1))', _x]
            ];
        };
    };
